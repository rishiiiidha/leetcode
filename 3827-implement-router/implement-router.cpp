...
private:
    ...
    unordered_map<int, deque<int>> counts; 
    queue<long long> q;
    ...
    int lowerBound(deque<int>& list, int target) {
        return (int)(lower_bound(list.begin(), list.end(), target) - list.begin());
    }

    int upperBound(deque<int>& list, int target) {
